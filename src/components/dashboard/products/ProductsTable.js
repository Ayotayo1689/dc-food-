...
const ProductsTable = () => {
	...
	const deleteQoute = async (id) => {
		...
		try {
			const resp = await axiosInstance.delete(`${endpoints.getProducts}${id}`, {
				headers: {
		...
		<>
					...
					<Tbody>
						{filteredProducts.map((row, index) => (
							<Tr fontWeight="500" fontSize="14px" color="#4B465C" key={index}>
								...
								<Td className="admin-table-font">{row?.name}</Td>
								<Td className="admin-table-font">
								...
								<Td className="admin-table-font">{row?.stock}</Td>
								<Td className="admin-table-font">
									<Text
										className="admin-table-font"
										...
										color={row.status === "selling" ? "#28C76F" : "white"}
									>
										{row?.status}
									</Text>
								</Td>
			...
			<Flex mb="50px" mt="30px" align="center" justify="space-between">
				<Text className="admin-font" color="#343330" fontWeight="600">
					Showing {indexOfFirstProduct + 1} - {indexOfLastProduct} of {count}
				</Text>
				<HStack>
					<IconButton
